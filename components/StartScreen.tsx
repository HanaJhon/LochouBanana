...
const StartScreen: React.FC<StartScreenProps> = ({ onFileSelect, onImageGenerated, onTemplateSelect, onShowTemplateLibrary }) => {
  ...
  const handleGenerate = async () => {
    ...
    try {
        const urls = await generateImageFromText(generationPrompt, aspectRatio, imageSize, imageCount, referenceImage || undefined);
        if (urls.length === 1) {
  ...
  const resolutions: { name: string; value: string }[] = [
    { name: '1K', value: '1K' },
    { name: '2K', value: '2K' },
    { name: '4K', value: '4K' },
  ];
  ...
  return (
    <div className="w-full max-w-6xl mx-auto px-4 py-8 flex flex-col gap-12 animate-fade-in relative">
      ...
      <div className="text-center space-y-6 py-8">
        <motion.h1 
            ...
            className="text-6xl md:text-8xl font-black tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-white via-gray-200 to-gray-400 drop-shadow-lg"
        >
          LochouBanana
        </motion.h1>
        <motion.div 
        ...
        >
             <h2 className="text-3xl md:text-4xl font-bold">
                <span className="text-gradient">化繁为简</span>，释放无限创意
            </h2>
             <p className="text-gray-400 text-lg max-w-2xl">
                至强改图模型 & AI 创意套件。从文本生成、智能修图到时空穿越。
                <a href="https://nb.kuai.host" className="text-blue-400 hover:text-blue-300 underline ml-2 decoration-blue-500/30">免费 API 部署 &raquo;</a>
            </p>
        </motion.div>
      </div>
            ...
            <div className="bg-gray-900/50 rounded-[22px] p-6 flex flex-col gap-5 h-full relative">
                ...
                <div className="flex flex-col gap-3">
                  <div className="flex flex-col gap-2">
                    <label className="text-xs font-semibold text-gray-400 uppercase tracking-wider">参考图 (可选)</label>
                    <div className="flex items-center gap-4">
                      <button
                        onClick={() => refInputRef.current?.click()}
                        className="flex flex-col items-center justify-center w-24 h-24 rounded-xl border-2 border-dashed border-white/10 bg-black/40 hover:bg-black/60 hover:border-purple-500/50 transition-all group overflow-hidden relative"
                      >
                        {referenceImageUrl ? (
                          <>
                      ...
                      {referenceImageUrl && (
                        <button 
                          ...
                          className="px-3 py-1.5 rounded-lg bg-red-500/10 border border-red-500/20 text-red-400 text-xs hover:bg-red-500/20 transition-all font-medium"
                        >
                          移除
                        </button>
                      )}
                  ...
                  <textarea
                      ...
                      className="w-full bg-black/30 border border-white/10 rounded-xl p-4 text-gray-100 focus:ring-2 focus:ring-purple-500/30 focus:border-purple-500/50 outline-none transition-all h-36 resize-none text-lg leading-relaxed placeholder-gray-600"
                ...
                <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                        <label className="text-xs font-semibold text-gray-400 uppercase tracking-wider">分辨率</label>
                        <div className="flex flex-wrap gap-2">
                            {resolutions.map(({ name, value }) => (
                                <button
                                    key={value}

...
const getGoogleAI = (): GoogleGenAI => {
    ...
    if (!aiInstance || apiKey !== lastUsedApiKey || baseUrl !== lastUsedBaseUrl) {
      try {
        // Use 'any' to bypass strict typing if the installed SDK version definitions 
        // haven't caught up with the 'baseUrl' property, though it is supported in the logic.
        const config: any = { apiKey };
        if (baseUrl) {
            // The @google/genai SDK uses 'baseUrl' to override the endpoint.
            // It usually expects the root domain (e.g., https://my-proxy.com).
            // The SDK appends /v1beta/models/... automatically.
...
export const generateImageFromText = async (prompt: string, aspectRatio: string, imageSize: string = '1K', imageCount: number = 1, referenceImage?: File): Promise<string[]> => {
    try {
        const ai = getGoogleAI();
        ...
        const generate = async () => {
            ...
            const response = await ai.models.generateContent({
                ...
                config: {
                    imageConfig: {
                        aspectRatio: aspectRatio === 'auto' ? undefined : aspectRatio as any,
        ...
        const promises = Array.from({ length: imageCount }, () => generate());
